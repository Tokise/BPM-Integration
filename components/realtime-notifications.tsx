...
export function RealtimeNotifications() {
    const { user, profile, addNotification } = useUser();
        ...
        const channel = supabase.channel('realtime_orders')
            .on(
                ...
                (payload: any) => {
                    ...
                    if (shopId && newOrder.shop_id === shopId) {
                        const message = `Order #${newOrder.id.slice(0, 8)} needs confirmation.`;
                        toast.success("New Order Received!", {
                            description: message,
                            action: {
                                label: "View",
                                onClick: () => router.push(`/seller/orders/${newOrder.id}`)
                            },
                        });
                        addNotification({
            ...
            .on(
                ...
                (payload: any) => {
                    ...
                    if (newOrder.customer_id === user.id) {
                        ...
                        if (newOrder.status !== oldOrder?.status || !oldOrder) {
                            // Map status to friendly text
                            const statusText = newOrder.status.replace('_', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase());
