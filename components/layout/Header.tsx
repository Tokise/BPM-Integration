                    ...
                    <div className="flex flex-1 items-center justify-end space-x-2">
                                ...
                                <div className="absolute right-0 mt-0 w-80 pt-2 z-[100] animate-in fade-in slide-in-from-top-2 duration-200">
                                    <div className="bg-white rounded-2xl border border-slate-100 shadow-xl overflow-hidden">
                                        <div className="flex items-center justify-between p-4 bg-slate-50 border-b border-slate-100">
                                            ...
                                            <Button variant="link" onClick={clearNotifications} className="h-auto p-0 text-[10px] font-bold text-primary">Clear All</Button>
                                        </div>
                                        <div className="max-h-[300px] overflow-y-auto scrollbar-thin">
                                            {notifications.length === 0 ? (
                                                ...
                                                notifications.map(notif => (
                                                    <div key={notif.id} className="p-4 border-b border-slate-50 hover:bg-slate-50/50 transition-colors last:border-0">
                                                        <div className="flex gap-3">
                                                            <div className={cn(
                                                                "h-8 w-8 rounded-full flex items-center justify-center flex-shrink-0",
                                                                notif.type === 'order' ? "bg-orange-50 text-orange-500" : "bg-blue-50 text-blue-500"
                                                            )}>
                                                                {notif.type === 'order' ? <Package className="h-4 w-4" /> : <Bell className="h-4 w-4" />}
                                                            </div>
                                                            <div className="min-w-0">
                                                                <p className="font-bold text-xs text-slate-900 leading-tight">{notif.title}</p>
                        ...
                        <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
                            ...
                            <SheetContent side="right" className="w-[300px] sm:w-[400px] flex flex-col p-6">
                                <SheetTitle className="text-2xl font-black mb-6">Your Cart</SheetTitle>
                                <div className="flex-1 overflow-y-auto pr-2 space-y-4 -mx-2 px-2 scrollbar-thin">
                                    {cartItems.length === 0 ? (
                                        <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-4">
                                            ...
                                            <Button variant="outline" className="rounded-xl" onClick={() => router.push('/')}>Start Shopping</Button>
                                        </div>
                                    ) : (
                                        cartItems.map((item) => (
                                            <div key={item.id} className="group relative flex flex-col gap-2 p-3 bg-white rounded-xl border border-slate-100 transition-all hover:border-primary/20">
                                                <div className="flex gap-3">
                                                    {/* Selection Checkbox */}
                                                    <button
                                                        ...
                                                        className={cn(
                                                            ...
                                                            item.selected ? "bg-primary border-primary text-black" : "border-slate-200 bg-slate-50"
                                                        )}
                                                    >
                                                ...
                                                <div className="flex items-center justify-between pt-1.5 border-t border-slate-50">
                                                    <span className="text-[8px] font-bold text-slate-400 uppercase">Quantity</span>
                                    ...
                                    <div className="border-t pt-6 pb-6 space-y-4 bg-white mt-auto sticky bottom-0">
                                        <div className="flex justify-between items-end px-1">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-xl font-black text-slate-900">{totalAmount.toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })}</span>
                                            </div>
                                            <span className="text-xs font-bold text-slate-500">{cartItems.filter(i => i.selected).reduce((acc, i) => acc + i.quantity, 0)} Items Selected</span>
                                        </div>
                                        <Button
                        ...
                        {loading ? (
                            ...
                            >
                                <Button variant="ghost" className="flex items-center gap-2 px-2 hover:bg-slate-100 rounded-xl cursor-pointer">
                                    ...
                                    <div className="hidden lg:flex flex-col items-start leading-none gap-0.5">
                                        <span className="font-bold text-sm text-slate-700">Account</span>
                                        {profile?.role && profile.role !== 'customer' && (
                                            <span className="text-[8px] font-black text-primary uppercase tracking-widest">{profile.role}</span>
                                        )}
                                    </div>
                                </Button>
                            ...
                            <Link href="/auth/sign-in">
                                <Button variant="ghost" className="hidden md:flex cursor-pointer hover:text-primary hover:bg-transparent font-semibold">
                                    Sign In
                                </Button>
                                        ...
                                        {user ? (
                                            <div className="space-y-4">
                                                <div className="flex items-center gap-3 px-2">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="font-black text-slate-900 truncate text-sm">{user.email?.split('@')[0]}</p>
                                                        <p className="text-[10px] font-bold text-slate-400 truncate uppercase tracking-widest leading-none">{user.email}</p>
                                                    </div>
                                                </div>
                                            ...
                                            <Link href="/auth/sign-in" className="flex items-center gap-2 text-lg font-semibold hover:text-primary cursor-pointer">
