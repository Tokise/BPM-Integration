...
export function SignUpForm({ signUpAction, searchParamsNext }: SignUpFormProps) {
    ...
    const requirementLabels = [
        { key: "length", label: "Exactly 8 characters" },
    ...
    return (
        ...
        >
            ...
            <div className="grid gap-2">
                <div className="flex items-center justify-between">
                    ...
                    <span className={cn(
                        ...
                        strength === 100 ? "text-emerald-500" : strength > 60 ? "text-amber-500" : "text-slate-400"
                    )}>
                ...
                <div className="space-y-2 mt-1 px-1">
                    <Progress
                        ...
                        indicatorClassName={cn(
                            ...
                            strength === 100 ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.3)]" :
                                strength > 60 ? "bg-amber-500" :
                                    strength > 20 ? "bg-orange-500" : "bg-red-500"
                        )}
                    />

                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 pt-1">
                        {requirementLabels.map((req, i) => {
                            const isMet = (requirements as any)[req.key];
                            return (
                                <div key={i} className="flex items-center gap-2 transition-all duration-300">
                                    <div className={cn(
                                        ...
                                        isMet ? "bg-emerald-500 border-emerald-500 text-white" : "border-slate-200 bg-white"
                                    )}>
                                        {isMet ? <Check className="h-2 w-2 stroke-[4px]" /> : <div className="h-1 w-1 bg-slate-300 rounded-full" />}
                                    </div>
                                    <span className={cn(
            ...
            <Button type="submit" className="w-full h-12 bg-primary text-black hover:bg-primary/90 font-black text-lg rounded-xl shadow-lg shadow-primary/20 transition-all border-none mt-2">
                Create Account
            </Button>
        </form>
    );
