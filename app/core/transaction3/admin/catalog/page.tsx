...
import { Package, Search, Filter, Database, Plus } from "lucide-react";
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="font-bold text-slate-500 uppercase text-[10px] tracking-[0.2em]">Global product master database</p>
                </div>
                <Button className="bg-amber-500 hover:bg-amber-600 text-white font-black rounded-xl h-12 shadow-lg shadow-amber-200 px-6 flex items-center gap-2">
                    <Plus className="h-5 w-5" /> Import Catalog
                </Button>
            </div>
            ...
            <Card className="border-none shadow-2xl shadow-slate-100 rounded-[32px] bg-white overflow-hidden p-12 text-center flex flex-col items-center">
                ...
                <h3 className="text-xl font-black text-slate-900">Admin Catalog Master</h3>
                <p className="text-slate-500 font-bold mt-2">Manage all system categories and product specifications.</p>
            </Card>
        </div>
    );
}
