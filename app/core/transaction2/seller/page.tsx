...
export default function SellerDashboard() {
    const { sellerProducts, sellerOrders, shop } = useUser();

    const stats = useMemo(() => {
        const productsCount = sellerProducts?.length || 0;
        const totalOrders = sellerOrders?.length || 0;
        const newOrders = sellerOrders?.filter(o => o.status === 'processing' || o.status === 'to_ship').length || 0;
        ...
        const dailyRevenue = sellerOrders?.reduce((acc, order) => {
        ...
        return {
            dailyRevenue: dailyRevenue.toLocaleString('en-PH', { style: 'currency', currency: 'PHP', maximumFractionDigits: 0 }),
    ...
    return (
        <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { label: "Daily Revenue", value: stats.dailyRevenue, change: "+8.5%", icon: DollarSign, color: "text-emerald-500", bg: "bg-emerald-50" },
                    { label: "New Orders", value: stats.newOrders, change: "+12.2%", icon: ShoppingBag, color: "text-amber-500", bg: "bg-amber-50" },
                    { label: "Active Products", value: stats.activeProducts, icon: Package, color: "text-blue-500", bg: "bg-blue-50" },
                    { label: "Avg Rating", value: stats.avgRating, icon: Star, color: "text-purple-500", bg: "bg-purple-50" },
                ].map((stat, idx) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="border-none hover:bg-transparent">
                                    ...
                                    <TableHead className="text-[10px] font-black uppercase text-slate-400 tracking-widest text-right">Earnings</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {recentSales.length > 0 ? (
                                    recentSales.map((order) => (
                                        <TableRow key={order.id} className="border-slate-50 hover:bg-slate-50/50 transition-colors group cursor-pointer">
                                            <TableCell className="font-bold py-4">{order.order_number}</TableCell>
                                            <TableCell>
                                                <span className="px-3 py-1 bg-blue-50 text-blue-500 text-[10px] font-black uppercase rounded-lg border border-blue-100">{order.status}</span>
                                            </TableCell>
                                            <TableCell className="font-black text-emerald-500 text-right">
                                                {order.total_amount.toLocaleString('en-PH', { style: 'currency', currency: 'PHP', maximumFractionDigits: 0 })}
                                            </TableCell>
                                        </TableRow>
                                    ))
                                ) : (
                                    <TableRow>
                                        <TableCell colSpan={3} className="text-center py-10 text-slate-400 font-bold uppercase text-[10px] tracking-widest">No recent sales</TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                        </Table>
                ...
                <Card className="border-none shadow-2xl shadow-slate-100 rounded-[32px] p-8 bg-white overflow-hidden flex flex-col items-center justify-center text-center gap-6">
                    ...
                    <div>
                        <h3 className="text-xl font-black text-slate-900">Store Performance</h3>
                        <p className="text-slate-500 mt-2 text-sm font-medium">Monitoring your store's reach and growth in real-time.</p>
                    </div>
                    <div className="w-full pt-6 border-t border-slate-50 grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p className="text-xl font-black text-slate-900">{stats.activeProducts}</p>
                        </div>
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Pending Orders</p>
                            <p className="text-xl font-black text-slate-900">{stats.newOrders}</p>
                        </div>
                    </div>
                </Card>
            </div>
