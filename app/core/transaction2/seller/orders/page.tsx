...
export default function SellerOrdersPage() {
    const { user, shopId: contextShopId, sellerOrders, refreshSellerOrders } = useUser();
    const router = useRouter();
    const [loading, setLoading] = useState(!sellerOrders);
    ...
    useEffect(() => {
        if (shopId) {
            refreshSellerOrders().finally(() => setLoading(false));
        }
    }, [shopId]);
                            ...
                            <TableBody>
                                {currentOrders.map((order) => (
                                    <TableRow
                                        ...
                                        onClick={() => router.push(`/core/transaction2/seller/orders/${order.id}`)}
                                    >
                            ...
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                                <Button
                                    ...
                                    className={cn(
                                        ...
                                        currentPage === page ? "bg-slate-100 text-slate-900" : "bg-transparent text-slate-400"
                                    )}
                                >
                                    {page}
                                </Button>
