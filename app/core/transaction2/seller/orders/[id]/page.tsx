...
import { getSellerOrder, updateSellerOrderStatus } from "@/app/actions/seller";
...
export default function SellerOrderDetailPage() {
    ...
    const fetchOrderDetails = async (showMainLoader = true) => {
        if (showMainLoader) setLoading(true);
        const result = await getSellerOrder(orderId);

        if (!result.success || !result.data) {
            console.error("Error fetching order:", result.error);
            toast.error("Failed to load order details: " + (result.error || "Unknown error"));
        } else {
        ...
        <div className="space-y-8 max-w-5xl mx-auto pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <h1 className="text-3xl font-black text-slate-900 tracking-tighter">Order #{order.id.slice(0, 8)}</h1>
                            <Badge className={`
                ...
                <div className="flex gap-3">
                    {order.status === 'to_pay' && (
                        <Button
                            onClick={() => handleUpdateStatus('to_ship')}
                            disabled={updating}
                        ...
                        >
                            {updating ? <Loader2 className="h-4 w-4 animate-spin" /> : "Confirm Payment"}
                        </Button>
                    )}
                    {order.status === 'to_ship' && (
                        <Button
                            onClick={() => handleUpdateStatus('to_receive')}
                            disabled={updating}
                        ...
                        >
                            {updating ? <Loader2 className="h-4 w-4 animate-spin" /> : "Ship Order"}
                        </Button>
                    )}
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="border-slate-50 hover:bg-transparent">
                                        <TableHead className="font-bold text-xs uppercase text-slate-400 pl-6">Product</TableHead>
                                        <TableHead className="font-bold text-xs uppercase text-slate-400 text-center">Qty</TableHead>
                                        <TableHead className="font-bold text-xs uppercase text-slate-400 text-right pr-6">Total</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <div className="h-16 w-16 bg-slate-50 rounded-xl overflow-hidden border border-slate-100 flex-shrink-0">
                                                        {item.product?.images?.[0] ? (
                                                            ...
                                                            <div className="h-full w-full flex items-center justify-center text-slate-300 font-bold text-xs">IMG</div>
                                                        )}
                                                    </div>
                                                    <div>
                                                        <p className="font-bold text-slate-900 text-sm line-clamp-2">{item.product?.name || item.product_name}</p>
                                                        <p className="text-xs text-primary font-bold mt-1">
                                                            {(item.price_at_purchase || 0).toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })}
                                                        </p>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            <TableCell className="text-center font-bold text-slate-600">x{item.quantity}</TableCell>
                                            <TableCell className="text-right font-black text-slate-900 pr-6">
                                                {((item.price_at_purchase || 0) * item.quantity).toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })}
                                            </TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                    ...
                    <Card className="border-none shadow-xl shadow-slate-100 rounded-[32px] overflow-hidden bg-white p-6">
                        <div className="flex flex-col gap-4">
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <span className="font-bold text-slate-900">{(order.total_amount - (order.shipping_fee || 0)).toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="font-bold text-slate-500">Shipping Fee</span>
                                <span className="font-bold text-slate-900">{(order.shipping_fee || 0).toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })}</span>
                            </div>
                            <Separator className="bg-slate-100" />
                            <div className="flex justify-between items-center">
                                <span className="font-black text-slate-900 text-lg">Total Amount</span>
                                <span className="font-black text-primary text-2xl">{order.total_amount.toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })}</span>
                            </div>
                        </div>
                    </Card>
                </div>
                    ...
                    <Card className="border-none shadow-xl shadow-slate-100 rounded-[32px] overflow-hidden bg-white">
                        ...
                        <CardContent className="p-6 space-y-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="font-black text-slate-900">{customer?.full_name || 'Guest User'}</p>
                                    <p className="text-xs font-bold text-slate-400">Customer ID: {order.customer_id.slice(0, 8)}</p>
                                </div>
                            </div>
                            <div className="space-y-3 pt-2">
                                <div className="flex items-center gap-3 text-sm">
                                    <Mail className="h-4 w-4 text-slate-400" />
                                    <span className="font-medium text-slate-600">{customer?.email || 'No email provided'}</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent className="p-6 space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-2 font-black text-slate-900">
                                    ...
                                    <Badge variant="outline" className={`ml-auto ${order.payment_status === 'paid' ? 'text-green-600 bg-green-50 border-green-200' : 'text-amber-600 bg-amber-50 border-amber-200'}`}>
