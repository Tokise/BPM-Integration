...
export default function AddProductPage() {
    ...
    const { user, refreshSellerProducts } = useUser();
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                console.error("Unexpected Category Error:", err);
            }
        };
        fetchData();
    }, [user?.id]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.name || !formData.price || !formData.stock_qty || !formData.category_id) {
        ...
        try {
            ...
            router.push('/core/transaction2/seller/products');
        } catch (error: any) {
    ...
    return (
        <div className="max-w-4xl mx-auto pb-20 px-4">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-3xl font-black tracking-tighter text-slate-900">Add New Product</h1>
                    <p className="font-bold text-slate-400 uppercase text-[10px] tracking-[0.2em]">Listing your product in the marketplace</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                <Card className="p-8 border-none shadow-2xl shadow-slate-200/50 rounded-[40px] bg-white">
                    <div className="grid gap-8">
                        ...
                        <div className="grid md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Stock Quantity *</Label>
                                <Input
                                    type="number"
                ...
                <Card className="p-8 border-none shadow-2xl shadow-slate-200/50 rounded-[40px] bg-white">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="text-lg font-black text-slate-900 tracking-tight">Product Images</h3>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <Label className="aspect-square rounded-[32px] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-slate-50 transition-colors group">
                            {uploading ? <Loader2 className="h-6 w-6 animate-spin text-amber-500" /> : <Plus className="h-8 w-8 text-slate-200 group-hover:text-amber-500 transition-colors" />}
