...
export default function ProductManagementPage() {
    const { user, shopId: contextShopId, shop: contextShop, sellerProducts, refreshSellerProducts } = useUser();
    ...
    const bestSeller = products.length > 0 ? products[0].name : "None";
    const avgReview = "4.6";
    return (
        <div className="space-y-10 max-w-7xl mx-auto pb-20 px-4 md:px-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="font-bold text-slate-400 uppercase text-[10px] tracking-[0.2em]">Add, track, and manage your shop listings</p>
                </div>
                <div className="flex flex-col sm:flex-row items-center gap-4">
                    ...
                    <Button
                        onClick={() => router.push('/core/transaction2/seller/products/add')}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <Card className="border-none shadow-xl shadow-slate-100 rounded-[32px] p-8 bg-white overflow-hidden group hover:scale-[1.02] transition-transform">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">In Stock</p>
                            <p className="text-3xl font-black text-slate-900">{inStock}</p>
                        </div>
                    </div>
                </Card>
                ...
                <Card className="border-none shadow-xl shadow-slate-100 rounded-[32px] p-8 bg-white overflow-hidden group hover:scale-[1.02] transition-transform">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p className="text-xl font-black text-slate-900 truncate max-w-[120px]">{bestSeller}</p>
                        </div>
                    </div>
                </Card>
                ...
                <Card className="border-none shadow-xl shadow-slate-100 rounded-[32px] p-8 bg-white overflow-hidden group hover:scale-[1.02] transition-transform">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Avg Review</p>
                            <p className="text-3xl font-black text-slate-900">{avgReview}</p>
                        </div>
                    </div>
                </Card>
            </div>
            ...
            <Card className="border-none shadow-2xl shadow-slate-200/50 rounded-[40px] p-10 bg-white overflow-hidden">
                <div className="flex items-center justify-between mb-8">
                    <h2 className="text-2xl font-black text-slate-900 tracking-tighter">All Listings</h2>
                    <Button variant="ghost" className="rounded-xl h-10 px-4 font-bold text-slate-500 gap-2 border border-slate-50">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="border-none hover:bg-transparent">
                                <TableHead className="text-[10px] font-black uppercase text-slate-400 tracking-widest h-12">Product</TableHead>
                                <TableHead className="text-[10px] font-black uppercase text-slate-400 tracking-widest h-12">Category</TableHead>
                                <TableHead className="text-[10px] font-black uppercase text-slate-400 tracking-widest h-12">Price</TableHead>
                                <TableHead className="text-[10px] font-black uppercase text-slate-400 tracking-widest h-12">Stock</TableHead>
                                <TableHead className="text-[10px] font-black uppercase text-slate-400 tracking-widest h-12">Status</TableHead>
                                <TableHead className="text-[10px] font-black uppercase text-slate-400 tracking-widest h-12 text-right">Actions</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {filteredProducts.map((p) => {
                                ...
                                return (
                                    <TableRow key={p.id} className="border-slate-50 hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <TableCell>
                                            <span className="text-sm font-bold text-slate-500">{p.category?.name || "Uncategorized"}</span>
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <span className={cn(
                                                "font-bold",
                                                isOutOfStock ? "text-red-500" : isLowStock ? "text-amber-500" : "text-slate-900"
                                            )}>
                                                {p.stock_qty}
                                            </span>
                                        </TableCell>
                                        ...
                                        <TableCell className="text-right">
                                            <Button variant="ghost" className="h-10 w-10 rounded-xl p-0 hover:bg-slate-100 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                        {[1, 2].map(i => (
                            <Button key={i} variant="outline" className={cn(
                                ...
                                i === 1 ? "bg-slate-100 text-slate-900" : "bg-transparent text-slate-400"
                            )}>
                                {i}
                            </Button>
                        ))}
