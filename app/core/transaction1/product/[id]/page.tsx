...
export default function ProductDetailPage() {
    ...
    const { getProductFromCache, cacheProduct } = useUser();
    ...
    useEffect(() => {
        ...
        async function fetchFromSupabase() {
            ...
            try {
                ...
                if (productData?.shop_id) {
                    ...
                    cacheProduct(productId, productData, shopData);
                }
            } catch (error) {
        ...
        fetchProductData();
    }, [productId, getProductFromCache, cacheProduct]);
    ...
    const fetchRelatedProducts = async () => {
        ...
        try {
            const { data, error } = await supabase
                ...
                .eq('status', 'active')
                .neq('id', productId)
                .or(`shop_id.eq.${product.shop_id},category_id.eq.${product.category_id}`)
                .limit(6);
    ...
    const handleBuyNow = async () => {
        const isAllowed = await protectAction(`/core/transaction1/checkout?productId=${productId}`);
        if (isAllowed) {
            router.push(`/core/transaction1/checkout?productId=${productId}`);
        }
    };
                ...
                <section>
                    ...
                    {reviewsLoading ? (
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            {reviews.map((review) => (
                                <Card key={review.id} className="p-6 rounded-[2rem] border-slate-100 bg-slate-50/30">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="h-10 w-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 font-black">
                                                {review.customer?.avatar_url ? (
                                                    <img src={review.customer.avatar_url} alt={review.customer.full_name} className="h-full w-full rounded-full object-cover" />
                                                ) : (
                                                    review.customer?.full_name?.[0] || 'U'
                                                )}
                                            </div>
                                            <div>
                                                <p className="font-black text-slate-900">{review.customer?.full_name || 'Anonymous'}</p>
                                    ...
                                    {review.media_urls && review.media_urls.length > 0 && (
                                        <div className="grid grid-cols-4 gap-2 mt-4">
                                            {review.media_urls.slice(0, 4).map((url: string, idx: number) => (
                ...
                <section className="pb-10">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        {product.category && (
                            <Button
                                ...
                                onClick={() => router.push(`/?category=${product.category.name}`)}
                            >
                                View Category
                            </Button>
                        )}
                    ...
                    {relatedProducts.length === 0 ? (
                        <Card className="p-12 rounded-[2rem] border-slate-100 bg-slate-50/30 text-center">
                            ...
                            <p className="text-slate-500 font-medium">No related products found</p>
                        </Card>
                    ) : (
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            {relatedProducts.map((relatedProduct) => (
                                <div
                                    ...
                                    onClick={() => router.push(`/core/transaction1/product/${relatedProduct.id}`)}
                                >
                                    ...
                                    <h4 className="font-bold text-xs text-slate-900 mb-1 truncate">{relatedProduct.name}</h4>
                                    <div className="flex items-center gap-2 mb-1">
                                        <p className="text-sm font-black text-primary">
                                            {relatedProduct.price.toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })}
                                        </p>
                                    </div>
                                    {relatedProduct.average_rating > 0 && (
                                        <div className="flex items-center gap-1">
                                            <StarRating rating={relatedProduct.average_rating} size="sm" showNumber={false} />
                                            <span className="text-[10px] text-slate-400">({relatedProduct.review_count})</span>
                                        </div>
                                    )}
                                </div>
                            ))}
