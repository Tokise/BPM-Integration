...
export default async function SetupAdmin() {
  ...
  if (!user) {
    return redirect(
      "/auth/sign-in?next=/auth/setup-admin",
    );
  }
  ...
  const promoteToAdmin = async () => {
    ...
    if (error) {
      ...
      return redirect(
        `/auth/setup-admin?error=${encodeURIComponent(error.message)}`,
      );
    }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-slate-100 p-8 text-center">
        ...
        <p className="text-slate-500 font-medium mb-8">
          Promote your account to access the
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 mb-8 flex gap-3 text-left">
          ...
          <p className="text-xs font-bold text-amber-800 leading-relaxed">
            This will grant full administrative
            access to all site controls,
