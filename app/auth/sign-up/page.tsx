...
export default async function SignUp(props: {
  ...
  const signUpWithEmail = async (
    ...
    if (!passwordRegex.test(password)) {
      const errorMsg =
        "Password must be exactly 8 characters long and include an uppercase letter, a lowercase letter, a number, and a special character.";
      return redirect(
        `/auth/sign-up?error=${encodeURIComponent(errorMsg)}`,
      );
    }
    ...
    const finalProtocol = cleanHost?.includes(
      "localhost",
    )
      ? "http"
      : protocol;
    const origin = cleanHost
      ? `${finalProtocol}://${cleanHost}/`
      : getURL();
    ...
    return redirect(
      "/auth/sign-in?message=Registration successful! Please check your email to confirm your account.",
    );
  };
  ...
  return (
    <div className="w-full min-h-screen lg:grid lg:grid-cols-2">
      <div className="hidden lg:flex flex-col items-center justify-center relative">
        ...
        <div className="relative z-10 max-w-lg text-center p-10">
          <img
            src="/logo.png"
            alt="Anec Global"
            className="mx-auto mb-8 h-32 w-auto opacity-80"
          />
          <h2 className="text-4xl font-bold mb-4 text-amber-900">
            Join the ANEC Community
          </h2>
          <p className="text-lg text-amber-800/80">
            Create an account to track your
            orders, save multiple addresses, and
            enjoy a personalized shopping
            experience.
          </p>
      ...
      <div className="flex items-center justify-center py-12 bg-white relative">
        <Link
          href="/"
          className="absolute top-8 left-8 lg:left-auto lg:right-8 flex items-center text-sm font-medium text-muted-foreground hover:text-primary transition-colors cursor-pointer"
        >
        ...
        <div className="mx-auto grid w-[400px] gap-6 px-4">
          <div className="flex flex-col gap-2 text-center lg:text-left">
            ...
            <p className="text-slate-500 font-medium">
              Enter your details below to get
              started
            </p>
          </div>
          ...
          <div className="mt-4 text-center text-sm text-muted-foreground">
            ...
            <Link
              href="/auth/sign-in"
              className="underline text-primary font-black hover:text-primary/80 transition-colors"
            >
              Log in
            </Link>
          </div>
